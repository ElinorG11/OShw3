 ...
 */
void clientPrintDynamic(int fd)
{
    rio_t rio;
    char buf[MAXBUF];
    int length = 0;
 ...
 */
void * clientSendDynamic(void *parameters)
{
...
struct thread_arguments *initThreadArgsDynamic(int fd, char *filename){
...
void destroyThreadArgsDynamic(struct thread_arguments *args) {
    free(args);
}
...
void testDynamic(char * host, int port) {
    ...
    for (int j = 0; j < NUMBER_OF_TESTS; ++j) {
        ...
        for (int i = 0; i < array_size; ++i) {
            arguments_array[i] = initThreadArgsDynamic(ConnFd[i],filename_array[i]);
        }
        ...
        for (unsigned int i=0; i<array_size; i++)
            pthread_create(&threads[i], NULL, clientSendDynamic, (void*)arguments_array[i]);
        ...
        for (int i = 0; i < array_size; ++i) {
            destroyThreadArgsDynamic(arguments_array[i]);
        }
