...
void * thread_workload(void * thread_id) {
    ...
    while(1) {
		...
		/* handle case where usec difference is negative (i.e. arrival 18.6 & dispatch 19.1). 
		 * usage: timersub(&left_operand, &right_operand, &res); */
		timersub(&execute_time, &arrival_time, &dispatch_interval);
		

        // request handling of a thread shouldn't block the others
        requestHandle(connfd, thread_stat, &dispatch_interval, &arrival_time);

        // executing critical section - accessing shared queue
        pthread_mutex_lock(&mutex);
...
{
    // From Piazza - no need to check for errors.
    if (argc < 5) {
